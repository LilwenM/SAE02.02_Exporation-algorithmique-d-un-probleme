class LePostierChinois : 
    ...
    def ajouter_arete(self, s1, s2) : 
        ...
        self._graphe[s1].append(s2)
        self._graphe[s2].append(s1)
        return
    ...
    def supprimer_arete(self, s1, s2) : 
        ...
        voisin = list(self._graphe[s1])
        for i in list(self._graphe[s1]) :
            if i == s2 :
                voisin.remove(i)
                break
        self._graphe[s1] = voisin
        
        voisin = list(self._graphe[s2])        
        for i in list(self._graphe[s2]) : 
            if i == s1 : 
                voisin.remove(i)
                break
    ...
    def cycle_eulerien(self) : 
        """ renvoie le cycle eulerien du graphe """
        ...
        #------Transforme en eulerien------
    
    def ajout_arete_sommetIsole(self) : 
        """ permet de lier les sommets impairs entre eux puis de lier le dernier au reste du graphe """
        ...
        if len(allSommet_isole) > 0 :
            ...
            if len(allSommet_degreImpair) > 0 : 
                ...
                if len(allSommet_degreImpair) > 0 : 
                    self.ajouter_arete(allSommet_isole[0], allSommet_degreImpair[0])
                elif len(allSommet_degrePair) > 0 :
                    self.ajouter_arete(allSommet_isole[0], allSommet_degrePair[0])
    ...
    def ajout_arete_sommetImpaire(self) : 
        ...
        while len(allSommet_degreImpair) > 0 : 
            ...
            for j in range(1, nbr_sommetImpair) : 
                if allSommet_degreImpair[i] != allSommet_degreImpair[j] : 
    ...
    def transforme_toEulerien(self) :
        """  """
        self.ajout_arete_sommetIsole()
        
        if self.est_eulerien() : 
            return
        
        self.ajout_arete_sommetImpaire()
